...
HeatPump::HeatPump() {
  ...
  currentSettings = {POWER_MAP[0], MODE_MAP[0],     21.0, FAN_MAP[0],
                     VANE_MAP[0],  WIDEVANE_MAP[2], false};
...
void HeatPump::setPowerSetting(bool setting) {
  if (wantedSettings.power == POWER_MAP[setting ? 1 : 0])
    return;
  GLOG_TRACE("MITSUBISHI", "Set Power (bool): %s", setting ? "ON" : "OFF");
  wantedSettings.power = POWER_MAP[setting ? 1 : 0];
}

const char *HeatPump::getPowerSetting() { return currentSettings.power; }

void HeatPump::setPowerSetting(const char *setting) {
  if (wantedSettings.power == setting)
    return;
  GLOG_TRACE("MITSUBISHI", "Set Power (string): %s", setting);
...
void HeatPump::setModeSetting(const char *setting) {
  if (wantedSettings.mode == setting)
    return;
  GLOG_TRACE("MITSUBISHI", "Set Mode (string): %s", setting);
...
void HeatPump::setModeIndex(uint8_t modeIndex) {
  if (modeIndex < 5) {
    if (wantedSettings.mode == MODE_MAP[modeIndex])
      return;
    GLOG_TRACE("MITSUBISHI", "Set Mode (index): %d (%s)", modeIndex,
               MODE_MAP[modeIndex]);
...
void HeatPump::setTemperature(float setting) {
  ...
  if (!tempMode) {
    // Clamp against the endpoints, not against a lookup of the rounded value:
    // createPacket truncates when it indexes TEMP_MAP, so a rounding guard lets
    // [15.5, 16) through to a negative index.
    if (setting < TEMP_MAP[15]) setting = TEMP_MAP[15];
    else if (setting > TEMP_MAP[0]) setting = TEMP_MAP[0];
  } else {
...
void HeatPump::setRemoteTemperature(float setting) {
  ...
  if (setting > 0) {
    ...
    packet[6] = 0x00;
    packet[8] = 0x80; // MHK1 send 80, even though it could be 00, since
                      // ControlByte is 00
  }
  // add the checksum
...
void HeatPump::setFanSpeed(const char *setting) {
  if (wantedSettings.fan == setting)
    return;
  GLOG_TRACE("MITSUBISHI", "Set Fan Speed (string): %s", setting);
...
void HeatPump::setFanSpeedIndex(uint8_t fanIndex) {
  if (fanIndex < 6) {
    if (wantedSettings.fan == FAN_MAP[fanIndex])
      return;
    GLOG_TRACE("MITSUBISHI", "Set Fan Speed (index): %d (%s)", fanIndex,
               FAN_MAP[fanIndex]);
...
void HeatPump::setVaneSetting(const char *setting) {
  if (wantedSettings.vane == setting)
    return;
  GLOG_TRACE("MITSUBISHI", "Set Vane (string): %s", setting);
...
void HeatPump::setVaneIndex(uint8_t vaneIndex) {
  if (vaneIndex < 7) {
    if (wantedSettings.vane == VANE_MAP[vaneIndex])
      return;
    GLOG_TRACE("MITSUBISHI", "Set Vane (index): %d (%s)", vaneIndex,
               VANE_MAP[vaneIndex]);
...
void HeatPump::setWideVaneSetting(const char *setting) {
  if (wantedSettings.wideVane == setting)
    return;
  GLOG_TRACE("MITSUBISHI", "Set Wide Vane (string): %s", setting);
...
void HeatPump::setWideVaneIndex(uint8_t wideVaneIndex) {
  if (wideVaneIndex < 7) {
    if (wantedSettings.wideVane == WIDEVANE_MAP[wideVaneIndex])
      return;
    GLOG_TRACE("MITSUBISHI", "Set Wide Vane (index): %d (%s)", wideVaneIndex,
               WIDEVANE_MAP[wideVaneIndex]);
...
int HeatPump::CelsiusToFahrenheit(float tempC) {
  float temp =
      (tempC * 1.8) + 32; // round up if heat, down if cool or any other mode
  return (int)(temp + 0.5);
}
...
void HeatPump::setRoomTempChangedCallback(
...
void HeatPump::sendCustomPacket(byte data[], int packetLength) {
  ...
  packetLength = (packetLength > PACKET_LEN)
                     ? PACKET_LEN
...
int HeatPump::lookupByteMapIndex(const int valuesMap[], int len,
                                 int lookupValue) {
  for (int i = 0; i < len; i++) {
...
int HeatPump::lookupByteMapIndex(const char *valuesMap[], int len,
...
bool HeatPump::canSend(bool isInfo) {
  return (millis() - (isInfo ? PACKET_INFO_INTERVAL_MS
                             : PACKET_SENT_INTERVAL_MS)) > lastSend;
}
...
int HeatPump::readPacket() {
  ...
  if (_HardSerial->available() > 0) {
    ...
    if (!foundStart) {
      GLOG_WARN("MITSUBISHI", "Read: Timeout (No response from AC)");
    ...
    for (int i = 1; i < 5; i++) {
      uint32_t start = millis();
      while (_HardSerial->available() == 0 && (millis() - start < 100)) {
        ESP.wdtFeed();
        yield();
      }
    ...
    if (header[0] == HEADER[0] && header[2] == HEADER[2] &&
      ...
      if (dataLength > (PACKET_LEN - 6)) { // Header(5) + Checksum(1) = 6
        GLOG_ERROR("MITSUBISHI",
                   "CRITICAL ERROR: Packet dataLength %d exceeds buffer!",
                   dataLength);
        return RCVD_PKT_FAIL;
      }
      ...
      while (_HardSerial->available() == 0 && (millis() - startCheck < 100)) {
        ESP.wdtFeed();
        yield();
      }
      ...
      if (data[dataLength] == checksum) {
        ...
        if (packetCallback) {
          ...
          for (int i = 0; i < (dataLength + 1);
               i++) { // must be dataLength+1 to pick up checksum byte
            packet[(i + 5)] = data[i];
          }
        ...
        if (header[1] == 0x62) {
          ...
          case 0x02: { // setting information
            ...
            receivedSettings.power =
                lookupByteMapValue(POWER_MAP, POWER, 2, data[3]);
            receivedSettings.iSee = data[4] > 0x08 ? true : false;
            receivedSettings.mode = lookupByteMapValue(
                MODE_MAP, MODE, 5,
                receivedSettings.iSee ? (data[4] - 0x08) : data[4]);
            ...
            if (data[11] != 0x00) {
              ...
              receivedSettings.temperature =
                  lookupByteMapValue(TEMP_MAP, TEMP, 16, data[5]);
            }

            receivedSettings.fan = lookupByteMapValue(FAN_MAP, FAN, 6, data[6]);
            receivedSettings.vane =
                lookupByteMapValue(VANE_MAP, VANE, 7, data[7]);
            receivedSettings.wideVane =
                lookupByteMapValue(WIDEVANE_MAP, WIDEVANE, 7, data[10] & 0x0F);
            ...
            if (receivedSettings != currentSettings) {
              ...
              GLOG_TRACE("MITSUBISHI", "RECV: Settings Update (No change)");
              currentSettings = receivedSettings;
            }

            // if this is the first time we have synced with the heatpump, set
            // wantedSettings to receivedSettings
            if (firstRun || (autoUpdate && externalUpdate)) {
              if (firstRun)
                GLOG_INFO("MITSUBISHI",
                          "Initial sync successful. Settings locked.");
              wantedSettings = currentSettings;
              firstRun = false;
            }
          ...
          case 0x03: { // Room temperature reading
            ...
            if (data[6] != 0x00) {
              ...
              receivedStatus.roomTemperature =
                  lookupByteMapValue(ROOM_TEMP_MAP, ROOM_TEMP, 32, data[3]);
            }
            ...
            if ((statusChangedCallback || roomTempChangedCallback) &&
                (currentStatus.roomTemperature !=
                     receivedStatus.roomTemperature ||
                 currentStatus.outsideAirTemperature !=
                     receivedStatus.outsideAirTemperature)) {

              if (!isnan(receivedStatus.outsideAirTemperature)) {
                GLOG_TRACE(
                    "MITSUBISHI", "RECV: Room Temp -> %sC, Outside Temp -> %sC",
                    String(receivedStatus.roomTemperature, 1).c_str(),
                    String(receivedStatus.outsideAirTemperature, 1).c_str());
              } else {
                GLOG_TRACE("MITSUBISHI", "RECV: Room Temp -> %sC",
                           String(receivedStatus.roomTemperature, 1).c_str());
              }
              ...
              if (roomTempChangedCallback) { // this should be deprecated -
                                             // statusChangedCallback covers it
          ...
          case 0x05: { // timer packet
            ...
            receivedTimers.mode =
                lookupByteMapValue(TIMER_MODE_MAP, TIMER_MODE, 4, data[3]);
            receivedTimers.onMinutesSet = data[4] * TIMER_INCREMENT_MINUTES;
            receivedTimers.onMinutesRemaining =
                data[6] * TIMER_INCREMENT_MINUTES;
            receivedTimers.offMinutesSet = data[5] * TIMER_INCREMENT_MINUTES;
            receivedTimers.offMinutesRemaining =
                data[7] * TIMER_INCREMENT_MINUTES;

            // callback for status change
            if (statusChangedCallback &&
                currentStatus.timers != receivedTimers) {
              GLOG_INFO("MITSUBISHI", "RECV: Timers Updated");
              currentStatus.timers = receivedTimers;
              statusChangedCallback(currentStatus);
            } else {
              currentStatus.timers = receivedTimers;
