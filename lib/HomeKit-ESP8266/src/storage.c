...
// A record is occupied if its magic matches, and deletion below clears the whole
// record including the magic. `active` is deliberately not consulted: the older
// Mixiaoxiao layout wrote that byte as 0, so gating reads on it makes every
// pairing created before this library was installed disappear. Keep this test
// identical to _find_empty_block's, or occupancy and reuse will disagree.
static inline bool pairing_present(const pairing_data_t *d) {
    return memcmp(d->magic, hap_magic, sizeof(d->magic)) == 0;
}
...
static void dump_pairing_data(int index, pairing_data_t *data) {
    if (!pairing_present(data)) return;
...
int homekit_storage_reset_pairing_data() {
    ...
    for (int i = 0; i < MAX_PAIRINGS; i++) {
        ...
        if (!memcmp(data.magic, hap_magic, sizeof(data.magic))) {
            ...
            // Clear every record, not just the first. The caller reaches this
            // after the last admin pairing goes away and expects the table
            // emptied so pair setup can be offered again.
            if (!spiflash_write(PAIRINGS_ADDR + (sizeof(data) * i), (uint32_t *) &data, sizeof(data))) {
...
static int compact_data() {
    ...
    for (int i = 0; i < MAX_PAIRINGS; i++) {
        ...
        if (!memcmp(src->magic, hap_magic, sizeof(hap_magic))) {
