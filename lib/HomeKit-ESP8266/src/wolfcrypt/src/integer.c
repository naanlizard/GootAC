...
//     winsize = 5;
//  }
  	if (winsize > ESP_INTEGER_WINSIZE){
  		winsize = ESP_INTEGER_WINSIZE;
  	}
#endif
...
#endif
{
...

  {
    ...
    if ((digs < MP_WARRAY) &&
        MIN(a->used, b->used) <=
        (1 << ((CHAR_BIT * sizeof (mp_word)) - (2 * DIGIT_BIT)))) {
    	//========= force use s_mp_mul 强制使用s_mp_mul
    	//实测fast_s_mp_mul_digs和s_mp_mul速度差不多
