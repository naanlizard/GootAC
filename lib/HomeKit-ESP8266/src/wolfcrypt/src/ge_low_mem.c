...
#ifdef HAVE_ED25519
#if  defined(ED25519_SMALL) || defined(ESP_GE_DOUBLE_SCALARMULT_VARTIME_LOWMEM) /* use slower code that takes less memory */
 ...
 * t is x*y.
 */
// All 32 bytes of each coordinate are required: under ED25519_SMALL a `ge` is
// byte[F25519_SIZE], so a short initializer zero-fills the rest and yields a
// corrupted base point, which makes every signature unverifiable. Upstream
// truncated these to 10 entries to silence an excess-initializer warning that
// only occurs in the !ED25519_SMALL build, where `ge` is int32_t[10].
