...
- **The threshold sliders span 5-40 C, wider than the unit accepts.** `TEMP_MAP` (`HeatPump.h`) is 16-31 C in 1 C steps, or 10-31 in the extended 0.5 C `tempMode`, and `setTemperature()` clamps into it. The slider ends say "do not condition on this side"; they are not commandable setpoints. 5-40 also exceeds the HAP defaults for these characteristics (cooling 10-35, heating 0-25). Changing either range needs a `config_number` bump in `homekit_ac.c`.
