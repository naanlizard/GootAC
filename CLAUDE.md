...
- **Smart Auto is firmware, not the AC's native auto.** A call starts when the room leaves the heating/cooling range and ends when it reaches that call's target, `auto_pull_c()` inside the range. That target is both the commanded setpoint and the release point; the idle staircase picks up at the same depth on release. See `ac_decision.cpp` for the pull constants.
...
- **The idle band delegates the fan to the unit in DRY** (`fan_idx` 0); in FAN it runs the same staircase on depth past the nearer target, so circulation winds down after a release and back up approaching either threshold; a room between a target and its threshold idles at 100%.
